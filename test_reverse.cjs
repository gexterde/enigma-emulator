...
let config = {
  ...
  leftRotor: { type: 'VIII', ring: charToNum('A')+1, start: charToNum('M'), current: charToNum('M') },
  middleRotor: { type: 'III', ring: charToNum('C')+1, start: charToNum('L'), current: charToNum('L') },
  rightRotor: { type: 'IV', ring: charToNum('U')+1, start: charToNum('W'), current: charToNum('W') },
...
console.log("Reversed rotors:", decrypted);
